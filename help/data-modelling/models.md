...
資料模型是一組結構描述、對象及其之間的連結。 您可以使用模型將資料庫的資料結合到您的對象中。
...
2. 填寫指定表單以定義連結。
   ...
   * **1-1**：來源表格的單一發生次數，最多可以擁有目標表格對應的單一發生次數。
   ...
   ![加號按鈕會反白顯示，讓您為模型建立多重聯結連結。](assets/models/multi-join.png){zoomable="yes"}
...
2. 選取Source加入旁的![](assets/models/do-not-localize/Smock_AddCircle_18_N.svg)按鈕，然後拖曳並引導箭頭朝向Target加入以建立連線。
